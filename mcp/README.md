...
2. Проверить, что сервер импортируется и видит инструменты:
   ...
   Ожидаемый вывод (9 инструментов Группы A + 5 Группы B):
   `['check_markers', 'check_patterns', 'check_structure', 'check_terms', 'get_book_info', 'get_chapter', 'get_chapter_plan', 'get_conflicts_table', 'get_glossary', 'get_pattern_details', 'get_patterns_for_phase', 'get_pending_promises', 'get_style_guide', 'verify_chapter']`.
...
mcp/
  ...
  tools/
    ...
    verify_tools.py      # Группа B — check_structure/markers/terms/patterns + verify_chapter
  tests/
...
- ✅ Группа B: `check_structure`, `check_markers`, `check_terms`,
  `check_patterns`, оркестратор `verify_chapter`.
- Группа B, далее: `check_promises` (обещания мостика отработаны в
  следующей главе) — ждёт появления второй смежной главы для смоук-теста.
