...
> **Карточка паттерна (`get_patterns_for_phase`) устойчива к двум
> форматам файлов.** В `patterns/` уживаются «новый» формат (32 файла:
> `id` = имя файла, `task_type`, `frequency_per_chapter`, тело с разделом
> `# Суть`) и «старый» (4 файла: `id` = код вида `T1`/`B3`, `category`
> вместо `task_type`, тело с разделом `# Описание`). Поэтому поля
> собираются с fallback'ом: `task_type ← task_type | category`,
...
> «Совместимость»; машиночитаемый `00_conflicts.md` ссылается на паттерны
> по имени файла.
