...
Дополнительно сервер выполняет **проверки** написанной главы
(Группа B) — структура, маркеры, термины, паттерны, обещания, стилгайд,
ссылки — и оркестратор `verify_chapter` со сводным вердиктом.
...
| `check_links` | существование локальных картинок и папок глав, на которые ссылается текст | проза ↔ файловая система (`chapters/chapter_NN/`) |
| `check_terminology` | единое написание терминов в прозе (нет ли запрещённых синонимов вместо канона) | проза ↔ контролируемый словарь `book_meta/terminology.yaml` |
...
- `check_links` коды: `missing_image` (error — локальная картинка из
  `![…](путь)` не найдена на диске относительно папки главы),
  `broken_chapter_ref` (warning — текст ссылается на «главу N» / «гл. N»,
  но папки `chapters/chapter_NN/` в репозитории нет; повторные ссылки на
  тот же номер не дублируются). Внешние картинки (`http://`, `https://`,
  `data:`, `//`) пропускаются. Сверяет прозу с файловой системой —
  `metadata.json` не нужен, работает и по черновику. `broken_chapter_ref`
  мягче картинки (warning, а не error): отсутствие папки главы часто
  значит «ещё не мигрирована», а не опечатку.
- `check_terminology` код: `noncanonical_term` (warning — в прозе
  встретился запрещённый вариант термина вместо канона). Источник —
  контролируемый словарь `book_meta/terminology.yaml` (`canon` +
  `variants` + необязательная `note`): один концепт = одно слово.
  Совпадение регистронезависимое и терпит дописанные русские окончания
  (`определитель` ловит `определителя`); инлайн-математика `$…$` и
  backtick-код вырезаются перед сканом; вариант, равный канону,
  игнорируется; дедуп по варианту — одна находка с первой строкой и
  суммарным числом вхождений. Это про **консистентность словаря**, не про
  разметку терминов (тем занят `check_terms`) и не про стиль
  (`check_styleguide`). Файла нет / он пуст / битый → проверка молчит;
  `metadata.json` не нужен, работает и по черновику.
- Если файла главы нет — ошибка «глава не написана». Если нет
  `metadata.json` — это находка `missing_metadata` (error), а не падение.
- **Строгость кодов настраивается** в `book_meta/checks_config.yaml` —
  необязательном файле **разреженных оверрайдов**: дефолты захардкожены в
  `verify_tools.py`, а конфиг переопределяет строгость только перечисленных
  кодов. Формат — `<проверка>: { <код>: <severity> }`, где `severity ∈
  error | warning | info | off`. Значение `off` убирает находку из отчёта
  совсем (не считается ни в `counts`, ни в вердикт). Применяется одинаково
  и к одиночным `check_*`, и к `verify_chapter`. Файла нет / битый YAML /
  неизвестный код или строгость → молча работают дефолты, проверки не
  падают. (Голый `off` YAML разбирает как `false`; работает и так, и в
  кавычках `"off"`.)
...
2. Проверить, что сервер импортируется и видит инструменты:
   ...
   Ожидаемый вывод (9 инструментов Группы A + 9 Группы B):
   `['check_links', 'check_markers', 'check_patterns', 'check_promises', 'check_structure', 'check_styleguide', 'check_terminology', 'check_terms', 'get_book_info', 'get_chapter', 'get_chapter_plan', 'get_conflicts_table', 'get_glossary', 'get_pattern_details', 'get_patterns_for_phase', 'get_pending_promises', 'get_style_guide', 'verify_chapter']`.
...
mcp/
  ...
  tools/
    ...
    verify_tools.py      # Группа B — check_structure/markers/terms/patterns/promises/styleguide/links/terminology + verify_chapter
  tests/
...
- ✅ Группа B полностью: `check_structure`, `check_markers`,
  `check_terms`, `check_patterns`, `check_promises`, `check_styleguide`,
  `check_links`, `check_terminology`, оркестратор `verify_chapter`.
- ✅ `book_meta/checks_config.yaml` — точечная настройка строгости кодов
  (разреженные оверрайды severity, `off` для полного скрытия находки).
- ✅ `book_meta/terminology.yaml` — контролируемый словарь (канон +
  запрещённые варианты) для `check_terminology`.
- Дальше — сборочный пайплайн автора (план → генерация → numeric →
  verify) и наполнение книги реальными главами.
