...
| `verify_chapter` | оркестратор: запускает все три проверки | сводный отчёт с вердиктом `ok`/`warn`/`fail` |
...
2. Проверить, что сервер импортируется и видит инструменты:
   ...
   Ожидаемый вывод (9 инструментов Группы A + 4 Группы B):
   `['check_markers', 'check_structure', 'check_terms', 'get_book_info', 'get_chapter', 'get_chapter_plan', 'get_conflicts_table', 'get_glossary', 'get_pattern_details', 'get_patterns_for_phase', 'get_pending_promises', 'get_style_guide', 'verify_chapter']`.
